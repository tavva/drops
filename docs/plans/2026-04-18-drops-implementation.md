...
**Goal:** Build a private static-site host at `drops.example.com` (app) and `content.drops.example.com` (served drops) per the design at `docs/plans/2026-04-18-drops-drops-design.md`.
...
describe('suggestSlug', () => {
  it('slugifies an email local-part', () => {
    expect(suggestSlug('Alice.Example+tag@example.com')).toBe('alice-example-tag');
  });
...
```sql
INSERT INTO allowed_emails (email) VALUES ('alice@example.com')
...
3. Stub redirects; complete "login" as `tester@example.com`.
...
5. Add custom domains `drops.example.com` and `content.drops.example.com` pointing at the service.
...
6. Production deploy on Railway serves `drops.example.com` and `content.drops.example.com` over HTTPS.
7. Manual smoke: log in as `alice@example.com`, create a drop by dragging a folder, view it at the content origin, re-upload, verify old content is gone.
