...
A private static-site host. Authenticated users drag a folder (or a zip) into the browser and get a live URL at `content.drops.example.com/:username/:dropname/`. Think Netlify Drop, but gated by Google OAuth and confined to a small group.
...
**Goals**
- Google OAuth only, restricted to an allowlist plus anyone on the `example.com` domain.
...
```
    ...
    drops.example.com                content.drops.example.com
             │                                         │
             ▼                                         ▼
             ┌─────────────────────────────────────┐
             │              Fastify                │
...
Content host (`content.drops.example.com`):
...
Note: `drops.example.com` and `content.drops.example.com` are the *same site* (same registrable domain), so `SameSite=Lax` alone does not prevent a cross-origin credentialed request between them. The real protections, layered:
...
- **Cookie scoping.** Both session cookies are host-only (no `Domain` attribute). `drops_session` goes only to `drops.example.com`; `drops_content_session` goes only to `content.drops.example.com`.
...
| `ALLOWED_DOMAIN` | Email domain auto-granted access (`example.com`) |
| `APP_ORIGIN` | `https://drops.example.com` |
| `CONTENT_ORIGIN` | `https://content.drops.example.com` |
...
```sql
-- Allowlist. Seeded with alice@example.com.
...
|---|---|---|
| `drops_session` | `drops.example.com` | App-host session |
| `drops_content_session` | `content.drops.example.com` | Content-host session (same session id) |
...
Route: `GET /:username/:dropname/*` on the **content host** (`content.drops.example.com`). Requests to this path pattern on the app host return 404.
...
- **Origin separation.** App (`drops.example.com`) and content (`content.drops.example.com`) are independent origins with independent cookies, so drop JavaScript has no same-origin access to `/app`.
