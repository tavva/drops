...
- The content host today is a single apex (`content.drops.example.com`) serving every drop under `/<user>/<drop>/*`. Same origin for every drop + a shared session cookie + no CSP on uploaded HTML = any drop's JavaScript can read any sibling the victim is authorised for.
...
- Wildcard `CNAME *.content.drops.example.com → <railway-target>` at the DNS provider.
- Railway custom domain `*.content.drops.example.com` added to the service (TLS handled by Railway via LE DNS-01 once DNS resolves).
...
- The apex `content.drops.example.com` has no remaining user-content surface, only 301 redirects. Helmet config stays strict there.
