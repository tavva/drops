...
Private static-site host for drops. The app origin (`drops.example.com`) handles auth and uploads; the content origin (`content.drops.example.com`) serves published drops to signed-in teammates.
...
| `GOOGLE_CLIENT_ID` / `GOOGLE_CLIENT_SECRET` | Google OAuth client for `example.com` workspace |
...
| `APP_ORIGIN` | e.g. `https://drops.example.com` |
| `CONTENT_ORIGIN` | e.g. `https://content.drops.example.com` |
...
4. Set `APP_ORIGIN=https://drops.example.com`, `CONTENT_ORIGIN=https://content.drops.example.com`, `SESSION_SECRET=$(openssl rand -hex 32)`, `ALLOWED_DOMAIN=example.com`, `GOOGLE_CLIENT_ID`/`SECRET`.
5. Deploy — Railway reads the Dockerfile, which runs migrations before starting the server.
6. Add both custom domains (`drops.example.com` and `content.drops.example.com`) in Railway → pointed at the same service.
