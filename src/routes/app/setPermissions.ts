// ABOUTME: POST /app/drops/:name/permissions sets the view mode; /domain-access toggles domain access
// ABOUTME: within emails mode. Validates against ('authed' | 'public' | 'emails'); non-owner returns 404 (no enumeration).
import type { FastifyPluginAsync } from 'fastify';
import { requireCompletedMember } from '@/middleware/auth';
import { findByOwnerAndName } from '@/services/drops';
import { setViewMode, setIncludeDomain } from '@/services/permissions';
...
export const setPermissionsRoute: FastifyPluginAsync = async (app) => {
  ...
  app.post('/app/drops/:name/domain-access', { preHandler: requireCompletedMember }, async (req, reply) => {
    const { name } = req.params as { name: string };
    if (!isValidSlug(name)) return reply.code(404).send('not_found');
    const drop = await findByOwnerAndName(req.user!.id, name);
    if (!drop) return reply.code(404).send('not_found');
    ...
    const body = (req.body ?? {}) as Record<string, string | undefined>;
    await setIncludeDomain(drop.id, body.include === '1');
    return reply.redirect(new URL(`/app/drops/${name}`, config.APP_ORIGIN).toString(), 302);
  });
};
